var dashboardApp = new Vue({
  el: '#dashboard', //el is the linking element to the part of the html page that we want to control. Its id should be dashboard.
  data: {
    project: {
      name : '',
      short_description: '',
  ...
  computed: {
    days_left: function () {
      return moment(this.project.target_date).diff(moment(), 'days') //what does the moment() func do?
    }
  },
  methods: {
    ...
    pretty_currency: function (val) {
      if (val < 1e3) {  //what is this?
        return '$ ' + val
      }

      if (val < 1e6) {
        return '$ ' + (val/1e3).toFixed(1) + ' k' //what is this?
      }

      return '$ ' + (val/1e6).toFixed(1) + ' M' //what is this?
    },
